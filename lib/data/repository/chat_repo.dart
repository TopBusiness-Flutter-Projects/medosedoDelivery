...
class ChatRepo {
  ...
  Future<Response> searchConversationList(String name) async {
    return apiClient.getData(AppConstants.searchConversationListUri +
        '?name=$name&limit=20&offset=1');
  }

  Future<Response> getMessagesList(int offset, String userType, int? id) async {
    return await apiClient.getData(
        '${AppConstants.messageListUri}$userType/$id?offset=$offset&limit=10');
  }

  Future<Response> searchChatList(String userType, String search) async {
    return await apiClient
        .getData('${AppConstants.chatSearch}$userType?search=$search');
  }

  Future<Response> sendMessage(
      String message, int? userId, String userType) async {
